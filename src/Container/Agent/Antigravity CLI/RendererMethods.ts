...
import antigravityCliArguments from './Arguments';
...
function checkEnvLine(line: string): 'set' | 'export' | 'var' | undefined {
  ...
  for (const arg of antigravityCliArguments) {
...
function getArgumentInfo(argName: string): {category: string; type: string; name: string} | undefined {
  for (const data of antigravityCliArguments) {
    if ('sections' in data) {
  ...
  if (settingsContent) {
    try {
      ...
      flattened.forEach(arg => {
        if (isValidArg(arg.name, antigravityCliArguments)) {
          settingsArgs.push(arg);
        }
      });
    } catch (error) {
...
export function parseStringToArgs(data: string): ChosenArgument[] {
  ...
  lines.forEach((line: string): void => {
    ...
    if (line.startsWith('agy')) {
      const clArg: string = line.substring(3).trim();
      if (!clArg) return;
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        const type = getArgumentType(info.name, antigravityCliArguments);
        if (type === 'CheckBox') {
    ...
    if (lineType === 'export' || lineType === 'set') {
      ...
      if (isValidArg(name, antigravityCliArguments)) {
...
function startInstall(stepper: InstallationStepper) {
  stepper.initialSteps(['Getting Started', 'Detect Existing', 'Antigravity CLI', 'All Done!']);
  stepper.starterStep({disableSelectDir: true}).then(() => {
    stepper.nextStep().then(() => {
      ...
      stepper.ipc.invoke('is_antigravity_cli_installed').then((isInstalled: boolean) => {
        if (isInstalled) {
          ...
          stepper.nextStep().then(() => {
            ...
            stepper.executeTerminalCommands(installCommand).then(() => {
              ...
              stepper.showFinalStep(
                ...
                'Antigravity CLI has been installed successfully.',
              );
            });
          });
        }
...
function startUpdate(stepper: InstallationStepper) {
  ...
  stepper.executeTerminalCommands('agy update').then(() => {
    ...
    stepper.showFinalStep(
      'success',
      'Antigravity CLI Updated Successfully!',
      'Antigravity CLI has been updated to the latest available version.',
    );
  });
}
...
const AntigravityCli_RM: CardRendererMethods = {
  cardInfo,
  parseStringToArgs,
  parseArgsToString,
...
export default AntigravityCli_RM;
